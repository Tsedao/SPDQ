...
class BinaryHeap(object):
    ...
    def _insert(self, priority, e_id):
        ...
        if self.check_full():
            min_p_ix = np.argmin(np.array(self.get_priority()))
            min_e_ix = self.get_e_id()[min_p_ix]
            self.cursor = self.e2p[min_e_ix]
            self.size -= 1
        else:
            self.cursor += 1

        self.priority_queue[self.cursor] = (priority, e_id)
        self.p2e[self.cursor] = e_id
        self.e2p[e_id] = self.cursor

        self.up_heap(self.cursor)
        return True
