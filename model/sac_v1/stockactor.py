...
class SACActor(object):
    ...
    def __init__(self, sess, config, feature_number, action_dim, window_size,num_mixtrue,learning_rate=0.01,
        ...
        self.layers = layers
        # Actor Network
        self.inputs, self.scaled_out, self.logprob, self.tau_tf = self.create_actor_network()

        self.network_params = tf.trainable_variables()

        # Target Network
        self.target_inputs, self.target_scaled_out, self.target_logprob, self.target_tau_tf = self.create_actor_network()
        ...
        self.tau_tf_gradient = tf.placeholder(self.dtype,[None,1],name='tau_tf_gradient')
        ...
        self.unnormalized_tau_tf_gradient = tf.gradients(
            self.tau_tf, self.network_params[:-6], self.tau_tf_gradient)

        self.unnormalized_actor_gradients = [sum(ele) for ele in zip(
            self.unnormalized_action_gradients[:-6],self.unnormalized_logprob_gradients[:-6], self.unnormalized_tau_tf_gradient)]
        self.unnormalized_actor_gradients += [sum(ele) for ele in zip(
            self.unnormalized_action_gradients[-6:],self.unnormalized_logprob_gradients[-6:])]
        ...
        self.optimize = tf.keras.optimizers.Adam(
                        self.lr_schedule).apply_gradients(zip([-1*g for g in self.actor_gradients], self.network_params))
    ...
    def create_actor_network(self):
        ...
        with tf.name_scope('det'):
            det = (1-tf.reduce_sum(z,axis=1,keepdims=True))*tf.math.reduce_prod(z,axis=1,keepdims=True)*(1/(tau_tf**action_dim))

        with tf.name_scope('log-likehood'):
            prob = tf.expand_dims(self.tf_mixtrue_density(pdf=self.tf_norm_pdf_multivariate,x=x,mus=mu,sigmas=sigma,weights=mixture_weights),axis=-1)

        with tf.name_scope('trans_llh'):
            logprob_z = tf.math.log(prob) - tf.math.log(det)
            # logprob = tf.expand_dims(tf.math.log(prob),axis=-1)


        return feature_input, z, logprob_z, tau_tf

    def train(self, inputs, a_gradient,l_gradient,t_gradient):
        """
        Args:
        ...
        self.sess.run(self.optimize, feed_dict={
            ...
            self.tau_tf_gradient: t_gradient
        })

    def predict(self, inputs):
        inputs = inputs[:, :, -self.window_size:, :]

        return self.sess.run([self.scaled_out,self.logprob, self.tau_tf], feed_dict={
            self.inputs: inputs
        })
    ...
    def predict_target(self, inputs):
        ...
        inputs = inputs[:, :, -self.window_size:, :]
        return self.sess.run([self.target_scaled_out,self.target_logprob,self.target_tau_tf], feed_dict={
