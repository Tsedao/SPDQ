...
class LowerControllerD3PG_RND(LowerController,D3PG):
    def __init__(self,
                 env,
                 val_env,
                 sess,
                 ...
                 obs_normalizer=False,
                 action_processor=None,
                 model_save_path='weights/hiro/lower_d3pgrnd.ckpt',
                 best_model_save_path='weights/best_hiro/lower_d3pgrnd.ckpt',
                 summary_path='results/hiro/'):
        LowerController.__init__(self, high_con=high_con,c=c)
        D3PG.__init__(self, env, val_env, sess, actor, critic, actor_noise, config,
                        policy_delay,obs_normalizer, action_processor,
        ...
        self.summary_ops_i, self.summary_vars_i = self.build_summaries('training_intrinsic')
        self.summary_vars_r_i, self.summary_vars_val_r_i = tf.Variable(0.,dtype=tf.float64,trainable=False), tf.Variable(0.,dtype=tf.float64,trainable=False)
        self.summary_ops_r_i = tf.summary.scalar("Episode_reward_i",self.summary_vars_r_i)
        self.summary_ops_val_r_i = tf.summary.scalar("Episode_reward_i_val",self.summary_vars_val_r_i)
    ...
    def train_one_step(self,previous_observation,s_g, ep_reward,ep_max_q, ep_loss,
        ...
        reward_intrinsic = self.rnd.compute_intrinsic_reward(np.expand_dims(previous_observation, axis=0))[0]
        print('intrinsic',reward_intrinsic)

        target_q_single = self.critic.predict_target(np.expand_dims(observation, axis=0),
                            self.actor.predict_target(np.expand_dims(observation, axis=0),n_sg) +
                            np.expand_dims(self.actor_noise(),axis=0))

        target_q_single_i = self.critic_intrinsic.predict_target(np.expand_dims(observation, axis=0),
                            self.actor.predict_target(np.expand_dims(observation, axis=0),n_sg) +
                            np.expand_dims(self.actor_noise(),axis=0))
        ...
        if done:
            y = reward*np.ones(shape=(1,self.num_quart))
            y_intrinsic = reward_intrinsic*np.ones(shape=(1,self.num_quart))
        else:
            y = reward + self.gamma * target_q_single
            y_intrinsic = reward_intrinsic + self.gamma * target_q_single_i


        TD_error = self.critic.compute_TDerror(np.expand_dims(previous_observation,axis=0),
                                               np.expand_dims(action, axis=0),
                                               y)[0]
        TD_error_i = self.critic_intrinsic.compute_TDerror(np.expand_dims(previous_observation,axis=0),
                                               np.expand_dims(action, axis=0),
                                               y_intrinsic)[0]
        ...
        s_g = n_sg
        rewards = reward
        rewards_i = reward_intrinsic
        print('n_sg:',n_sg)
        print('is_nan',np.isnan(observation).any())
        TD_errors = 0.5 * TD_error + 0.5 * TD_error_i
        n_sg = np.squeeze(n_sg,axis=0)
        # add to buffer
        # self.buffer.add(previous_observation, action, reward, done, observation)
        if self.buffer.size() >= self.batch_size:
            # batch update
            #s_batch, a_batch, r_batch, t_batch, s2_batch = self.buffer.sample_batch(batch_size)
            (s_batch, sg_batch, a_batch, r_batch,ri_batch, t_batch, s2_batch,sg2_batch), is_weights, indices = self.buffer.select(self.batch_size)

            noise = np.vstack([self.actor_noise() for i in range(self.batch_size)])
            target_q = self.critic.predict_target(s2_batch, self.actor.predict_target(s2_batch,sg2_batch)+noise)
            target_q_i = self.critic_intrinsic.predict_target(s2_batch, self.actor.predict_target(s2_batch,sg2_batch)+noise)
            y = []
            y_i = []
            TD_errors_list = []
            TD_errors_list_i = []
            ...
            for k in range(self.batch_size):
                if t_batch[k]:
                    y_tmp = r_batch[k] * np.ones(self.num_quart)
                    y_tmp_i = ri_batch[k] * np.ones(self.num_quart)
                else:
                    y_tmp = r_batch[k] + self.gamma * target_q[k,...]
                    y_tmp_i = ri_batch[k] + self.gamma * target_q_i[k,...]
                # Update the critic given the targets
                ...
                TD_error = self.critic.compute_TDerror(np.expand_dims(s_batch[k],axis=0),
                                                       np.expand_dims(a_batch[k],axis=0),
                                                       np.array([y_tmp]))[0]
                TD_error_i = self.critic_intrinsic.compute_TDerror(np.expand_dims(s_batch[k],axis=0),
                                                       np.expand_dims(a_batch[k],axis=0),
                                                       np.array([y_tmp_i]))[0]
                y.append(y_tmp)
                y_i.append(y_tmp_i)
                TD_errors_list.append(TD_error)
                TD_errors_list_i.append(TD_error_i)

            TD_errors_batch = np.array([0.5*c+0.5*i for c, i in zip(TD_errors_list,TD_errors_list_i)])
            ...
            predicted_q_value, q_loss,  _  = self.critic.train(
                                            s_batch, a_batch,
                                            np.reshape(y, (self.batch_size, self.critic.num_quart)),
                                            bias)

            predicted_q_value_i, q_loss_i,  _  = self.critic_intrinsic.train(
                                            s_batch, a_batch,
                                            np.reshape(y_i, (self.batch_size, self.critic.num_quart)),
                                            bias)

            _, rnd_loss = self.rnd.train(s2_batch)
            # Update the actor policy using the sampled gradient
            a_outs = self.actor.predict(s_batch,sg_batch)
            grads_c = self.critic.action_gradients(s_batch, a_outs)
            grads_i = self.critic_intrinsic.action_gradients(s_batch, a_outs)
            grads = [0.5*c+0.5*i for c, i in zip(grads_c,grads_i)]
            if step_counter % self.policy_delay == 0:
                self.actor.train(s_batch,sg_batch, grads)
                ...
                self.actor.update_target_network()
            self.critic.update_target_network()
            self.critic_intrinsic.update_target_network()
            ...
            summaries, summaries_i = self.sess.run([self.summary_ops,self.summary_ops_i], feed_dict = {
                                    self.summary_vars[0] : np.mean(q_loss),
                                    self.summary_vars[1] : np.amax(np.mean(predicted_q_value,axis=1)),
                                    self.summary_vars_i[0] : np.mean(q_loss_i),
                                    self.summary_vars_i[1] : np.amax(np.mean(predicted_q_value_i,axis=1))
            })

            [self.writer.add_summary(summary, self.training_max_step*epi_counter+self.n_step*step_counter
                                    ) for summary in summaries]
            [self.writer.add_summary(summary, self.training_max_step*epi_counter+self.n_step*step_counter
                                    ) for summary in summaries_i]
            self.writer.flush()

        return start_obs, n_sg, start_action, rewards, rewards_i, observation, done, TD_errors, ep_reward, ep_max_q, ep_loss
...
class HIRO(BaseModel):
    ...
    def train(self,verbose=True):
        ...
        if self.use_rnd:
            self.low_con.critic_intrinsic.update_target_network()
        ...
        for i in range(self.num_episode):
            ...
            for j in range(0,self.training_max_step,self.training_max_step_size):
                ...
                if not self.use_rnd:
                    ...
                    start_obs_w, n_sg, start_action, rewards_w, rewards_i, obs_w, done_w, TD_errors_w, ep_reward_w, ep_max_q_w ,ep_loss_w = self.low_con.train_one_step(
                    previous_observation,start_goal, ep_reward_w,ep_max_q_w, ep_loss_w,i, j)
                    ep_reward_i += rewards_i
                ...
                if not self.use_rnd:
                    ...
                    self.low_con.buffer.store((start_obs_w, start_goal, start_action, rewards_w, rewards_i,
                                               done_w, obs_w,n_sg),TD_errors_w)
                ...
                if (done_w and done_m) or j == self.training_max_step - self.training_max_step_size:
                    ...
                    if self.use_rnd:
                        reward_summary_i = self.sess.run(self.low_con.summary_ops_r_i,
                                                    feed_dict={
                                                    self.low_con.summary_vars_r_i : ep_reward_i
                                                    })
                        self.writer.add_summary(reward_summary_i,i)
