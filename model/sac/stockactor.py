...
class SACActor(object):
    ...
    def __init__(self, sess, config, feature_number, action_dim, window_size,num_mixture,learning_rate=0.01,
        ...
        self.inputs, self.scaled_out, self.logprob, self.test_prob, self.test_det, \
            self.test_mu, self.test_sigma, self.test_x, self.test_mw = self.create_actor_network()

        self.network_params = tf.trainable_variables()

        # Target Network
        self.target_inputs, self.target_scaled_out, self.target_logprob, self.test_target_prob, self.test_target_det, \
          self.test_target_mu, self.test_target_sigma, self.test_target_x,self.test_target_mw = self.create_actor_network()
    ...
    def create_actor_network(self):
        ...
        self.tau_tf = tf.constant(0.01,dtype=self.dtype)
        ...
        with tf.name_scope('mixture_weights'):
            ...
            mixture_weights = mixture_weights / tf.reduce_sum(mixture_weights, axis=-1,keepdims=True)
        # with tf.name_scope('tau'):
        ...
        with tf.name_scope('det'):
            det = tf.math.reduce_sum(tf.math.log(z),axis=1,keepdims=True)-action_dim*tf.math.log(self.tau_tf)
        ...
        with tf.name_scope('log-likehood'):
            ...
            logprob_z = tf.math.log(prob) - det
